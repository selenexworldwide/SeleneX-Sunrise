// Final comprehensive fix - replaces entire JavaScript section with working version
const fs = require('fs');
...
// Correct JavaScript from landing page (working version)
const CORRECT_JS = `    // Initialize Lucide icons
...
async function fixPage(pagePath) {
  try {
    ...
    // Find the reveal-on-scroll code to preserve it
    const revealPattern = /(document\.addEventListener\('DOMContentLoaded', \(\) => \{[\s\S]*?const observerOptions[\s\S]*?\}\);\s*})/;
    const revealMatch = content.match(revealPattern);
    const revealCode = revealMatch ? revealMatch[1] : '';
    
    // Find where the Lucide initialization starts (after reveal code)
    const lucideStart = content.indexOf('// Initialize Lucide icons');
    if (lucideStart === -1) {
    ...
    // Build new content: reveal code + correct dropdown JS
    const beforeScript = content.substring(0, revealMatch ? revealMatch.index + revealMatch[0].length : lucideStart);
